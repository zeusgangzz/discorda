...
exports.run = async (client, message, args) => {
    ...
    var data = sql.prepare(`SELECT * FROM punishments WHERE type = 'Warn' AND user = ${user.id}`).all()
    
    const embed = new Discord.MessageEmbed()
      .setColor(client.config.colour)
      .setTitle(client.l.moderation.warnings.history)
      .setDescription(client.l.moderation.warnings.user.replace('%USER%', user))  
    ...
    data.forEach(item => {
        ...
        if(item.status === "UW") status = `\n${client.l.moderation.warnings.unwarned}`
        pages.push([`[${item.id}] **${item.type}** (${item.start})`, `${client.l.moderation.warnings.staffMember.replace('%STAFF%',staff)}${status}\n${client.l.moderation.warnings.reason.replace('%REASON%',item.reason)}`])
    })
    ...
    embed.setFooter(`${client.l.moderation.warnings.page} ${page}/${maxpages}`)
