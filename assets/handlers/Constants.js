...
/**
 * 
 *  Zeltux does not own, nor claim to own or have created this package.
 * 
 *  It is used like any other package within this project that would 
 *  normally be in the node_modules folder.
 * 
 *  We have only made some small adjustments that require it to
 *  be in the assets folder, instead of the node_modules folder.
 * 
 *  Feel free to get in touch on our Discord to find out more.
 * 
 *  Discord - https://discord.zeltux.net
 * 
 */
...
config = loadFile(`./config/config.yml`)
l = loadFile(`./config/language.yml`)
...
exports.defaultGiveawayMessages = {
    giveaway: l.giveaways.gcreate.create,
    giveawayEnded: l.giveaways.gcreate.ended,
    inviteToParticipate: `${l.giveaways.gcreate.participate.replace('%EMOJI%', config.giveaways.reaction)}`,
    timeRemaining: `${l.giveaways.gcreate.time.replace('%DURATION%', '{duration}')}`,
    winMessage: `${l.giveaways.gcreate.winner.replace('%WINNERS%', '{winners}').replace('%PRIZE%', '{prize}')}`,
    embedFooter: 'Powered by the discord-giveaways package',
    noWinner: l.giveaways.gcreate.noWinner,
    winners: l.giveaways.gcreate.winners,
    endedAt: l.giveaways.gcreate.endsAt,
 ...
 *
 * @property {string} [storage='./data/giveaways.json'] The storage path for the giveaways.
...
exports.defaultManagerOptions = {
    ...
    default: {
        ...
        exemptPermissions: config.giveaways.ignoreIfHasPermission,
        exemptMembers: () => false,
        embedColor: config.colour,
